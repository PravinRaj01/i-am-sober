...
const AIObservability = () => {
  ...
  const interventionCount = logs?.filter(l => l.intervention_triggered).length || 0;

  // Group by function name for chart
  const functionUsage = logs?.reduce((acc, log) => {
    acc[log.function_name] = (acc[log.function_name] || 0) + 1;
    return acc;
  }, {} as Record<string, number>) || {};

  const functionChartData = Object.entries(functionUsage).map(([name, count]) => ({
    name: name.replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
    calls: count,
  }));

  // Tools usage
  const toolsUsage = logs?.reduce((acc, log) => {
  ...
  const toolsChartData = Object.entries(toolsUsage).map(([name, count]) => ({
