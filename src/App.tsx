...
const AppContent = () => {
  ...
  useEffect(() => {
    const checkToken = async () => {
      const dismissed = localStorage.getItem("ai-token-alert-dismissed");
      if (dismissed) return;
      ...
      if (!profile?.huggingface_token) {
        setTimeout(() => setShowTokenAlert(true), 2000);
      }
    };
  ...
  return (
    <>
      <AITokenAlert open={showTokenAlert} onOpenChange={setShowTokenAlert} />
      <Routes>
