...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Analyze this text and classify the primary emotional trigger. Respond with ONLY a JSON object: {"trigger": "one of: ${candidateLabels.join(', ')}", "confidence": number between 0 and 1, "topLabels": ["label1", "label2", "label3"], "topScores": [0.9, 0.05, 0.03]}. Text: "${text}"`;
