...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Running daily reminder check for hour: ${currentHour}`);
    ...
    for (const profile of profiles || []) {
      ...
      // Check if reminder time matches (within the hour)
      const reminderTime = prefs.daily_reminder_time || '09:00';
      const reminderHour = reminderTime.split(':')[0].padStart(2, '0') + ':00';
