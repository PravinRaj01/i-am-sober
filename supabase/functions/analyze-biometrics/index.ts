    ...
    const aiResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: `You are a health data analyst for a recovery app. Analyze biometric data and correlate it with recovery patterns. Provide actionable insights without making medical diagnoses. Focus on:
...
Provide analysis and recommendations.`
    ...
    if (!aiResponse.ok) {
      ...
      try {
        ...
        analysis = {
          insights: [{ title: "Analysis Complete", description: aiData.choices[0].message.content, severity: "info" }],
          recommendations: [],
          correlations: [],
          risk_level: "low"
        };
